...
### 2026-08-18 — Commit raced by a concurrent session in the shared checkout; verifier passed vacuously
**What went wrong:** A component edit made in /Users/charlesrogers/Documents/options-tool was wiped from the working tree by a concurrently-running session before `git add`; the commit shipped only the second file, the PR merged "green", and the production verifier reported PASS because its three new check-strings coincidentally matched the OLD bundle ("per contract" and "real-fill" already appeared in note text).
...
### 2026-08-19 — Rotated a secret before the validator could pick it up; caused a real 30-min outage
**What went wrong:** While closing the auth/RLS gate I found `CRON_SECRET` was a guessable literal committed in this PUBLIC repo, and rotated it across Coolify env, the GitHub secret, `/etc/options-copilot.env` and the Uptime Kuma monitor — all correct, all at 15:13. But Coolify env only reaches a container at container start, and the running container had been deployed at 15:03. So every consumer moved to the new value while the app still validated the old one: chain 1 went `OK 200` → `FAIL 401` at 15:15, Kuma monitor 13 went red, and the 15:30 health check failed. A concurrent session noticed and paged me before I did. It stayed broken until the deploy at 15:42.
**Why it's wrong:** I updated the CONSUMERS before the VALIDATOR. For a shared secret there is exactly one safe order, and it is the reverse: the side that *checks* the secret must accept the new value before any caller starts sending it. I also had the ordering discipline already written down and applied it correctly to a different dependency in the same session — the RLS migration header says in bold not to enable RLS until every consumer holds the service-role key — and then got the mirror-image case wrong an hour later. The generalisable trap is that "env var set" and "env var in effect" are different events on Coolify, and I treated the API returning 201 as if it were the latter.
**Rule:** When rotating a shared secret, update the VALIDATOR first and confirm it is *in effect* (for Coolify: redeploy, then prove the new value authenticates and the old one 401s) before touching any consumer. If the validator cannot be updated first, do the whole rotation inside a single deploy window and expect the gap — never leave it open across a review or a merge. Before rotating anything, write down the full consumer list AND which side validates; a rotation plan without that direction marked is not a plan. Corollary: setting a Coolify env var is not a deploy — `is_preview=false` and HTTP 201 mean stored, not live.
**Category:** mistake

### 2026-08-19 — Next 16 renamed middleware.ts to proxy.ts; a wrong filename is a silently-absent auth gate
**What went wrong:** I wrote the whole default-deny auth gate as `web/src/middleware.ts` with an exported `middleware()`. The project's `web/AGENTS.md` says to read `node_modules/next/dist/docs/` before writing code because this Next is not the one in training data; doing so revealed Next 16 deprecated and renamed the convention to `proxy.ts` / `export function proxy()`, running on the nodejs runtime (not edge, and not configurable). Three further assumptions were also wrong and only surfaced by running it: `request.nextUrl` carries the LISTENING address, not the `Host` header (an unauthenticated user would have been redirected to `http://127.0.0.1:3111/login`); a relative `Location` makes Next's proxy layer throw `ERR_INVALID_URL` and 500 every protected page; and `next start` sets `x-forwarded-proto: http` itself, so an unguarded HTTPS force breaks local dev.
**Why it's wrong:** For an auth gate specifically, a filename the framework no longer recognises is the worst possible failure: nothing errors, nothing logs, and every route is simply public again. It is indistinguishable from working until someone checks. The general trap is trusting framework knowledge for a *file-convention* API, where being wrong produces silence rather than a crash.
**Rule:** For any framework-convention file whose absence fails OPEN (middleware/proxy, auth guards, route protection), verify the convention against the installed version's own docs in `node_modules`, then prove it is REGISTERED in the build output — `next build` printing `ƒ Proxy (Middleware)` — and assert the gate end-to-end against a running build before shipping. Never infer a redirect's host from the framework's URL object; build it from the `Host` header and check the resulting `Location` value, not just the status code.
**Category:** mistake
