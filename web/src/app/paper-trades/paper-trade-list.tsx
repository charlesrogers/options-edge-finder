...
interface Stats {
  ...
  provenance?: {
    synthetic: { total: number; scored: number }
    live: { total: number; scored: number }
...
export function PaperTradeList() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] text-muted-foreground/70 mt-0.5">
            Hold-to-expiry outcomes of logged recommendations — not the copilot strategy, which
            buys back early.
          </p>
        </div>
        {/* Sort */}
      ...
      {/*
        Provenance banner. Every scored row below is a synthetic trade priced by
        backfill_paper_trades.py with Black-Scholes off stock history — the
        scoreboard describes that pricing model, not the strategy. Stated once,
        loudly, above the numbers rather than as a footnote beneath them.
        See results/013_paper_trade_audit.md.
      */}
      {stats.provenance && stats.provenance.live.scored === 0 && stats.provenance.synthetic.scored > 0 && (
        <div className="rounded-xl border border-amber-200 dark:border-amber-500/20 bg-amber-50/60 dark:bg-amber-500/5 px-5 py-4">
          <p className="text-[13px] font-semibold text-amber-800 dark:text-amber-300">
            Every scored trade below is synthetic
          </p>
          <p className="text-[12px] text-amber-800/80 dark:text-amber-300/80 mt-1 leading-relaxed">
            All {stats.provenance.synthetic.scored} scored rows were generated by the backfill
            script, which priced hypothetical trades with Black-Scholes off stock history rather
            than quoted option prices. They are a property of that pricing model and are not a
            track record. {stats.provenance.live.total} recommendation
            {stats.provenance.live.total !== 1 ? 's have' : ' has'} been logged off live option
            chains and none has reached expiry
            {stats.provenance.first_live_outcome_due
              ? `; the first outcomes are due ${stats.provenance.first_live_outcome_due}.`
              : '.'}
          </p>
        </div>
      )}
